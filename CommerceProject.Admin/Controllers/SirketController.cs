...
namespace CommerceProject.Admin.Controllers
{
    public class SirketController : BaseController
    {
        ...
        public SirketController(IIcerikAyarService iIcerikAyarService,
                                IKullaniciService iKullaniciService,
                                ISirketService iSirketService) : base(iIcerikAyarService,
                                                                      iKullaniciService)
        {
        ...
        {
            ViewBag.PageName = PageHelper.Pages.sirket_index;
            ViewBag.PageProperties = PageHelper.PageProperties.SetPageProperties("Sirket", "Arama", "Sirket Arama İşlemleri", "");
        ...
        {
            ViewBag.PageName = PageHelper.Pages.sirket_save;

            if (id == null)
            {
                ViewBag.PageProperties = PageHelper.PageProperties.SetPageProperties("Sirket", "Kayıt", "Şirket Kayıt İşlemleri", "");
                ViewBag.Sirket = new Sirket();
            }
            else
            {
                ViewBag.PageProperties = PageHelper.PageProperties.SetPageProperties("Sirket", "Güncelleme", "Şirket Güncelleme İşlemleri", "");
                ViewBag.Sirket = SirketService.FindBy(x => x.SirketId == id).SingleOrDefault();
            }
        ...
        {
            var sonucListesi = SirketService.FindBy(x =>
        ...
        #region Ajax Methods
        [HttpPost]
        public JsonResult KaydetGuncelle(Sirket sirket)
        {
            try
            {
                Sirket _sirket;
                ...
                {
                    _sirket = new Sirket();
                    _sirket.Aciklama = sirket.Aciklama;
                    _sirket.Adi = sirket.Adi;
                    _sirket.AktifMi = sirket.AktifMi;
                    _sirket.Sira = sirket.Sira;

                    SirketService.Add(_sirket);
                }
                else
                {
                    _sirket = SirketService.FindBy(x => x.SirketId == sirket.SirketId).SingleOrDefault();
                    ...
                    SirketService.Edit(_sirket);
                }

                var flag = SirketService.Save();

                if (flag)
                    return Json(_sirket.SirketId, JsonRequestBehavior.DenyGet);
        ...
        public JsonResult Sil(int id)
        {
            try
            {
                ...
                var _sirket = SirketService.FindBy(x => x.SirketId == id).SingleOrDefault();

                if (_sirket == null)
                    return Json(false, JsonRequestBehavior.DenyGet);

                _sirket.AktifMi = false;
        ...
        {
            var tempList = SirketService.FindBy(x =>
            (string.IsNullOrEmpty(aciklama) || x.Aciklama.Contains(aciklama)) &&
            (string.IsNullOrEmpty(adi) || x.Adi.Contains(adi)) &&
            (aktiflik == 2 || ((aktiflik == 0 && x.AktifMi == false) || (aktiflik == 1 && x.AktifMi == true))));
            ...
            var sonucListesi = tempList.OrderBy(x => x.SirketId).Skip((sayfaSirasi - 1) * sayfaSayisi).Take(sayfaSayisi).ToList();
