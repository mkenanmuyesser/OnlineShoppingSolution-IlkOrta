...
{
    public class HaberBulteniController : BaseController
    {
        #region Excel Action
        ...
        public HaberBulteniController(IIcerikAyarService iIcerikAyarService,
                                      IKullaniciService iKullaniciService,
                                      IHaberBulteniAbonelikService iHaberBulteniAbonelikService) : base(iIcerikAyarService,
                                                                                                        iKullaniciService)
        {
        ...
        {
            ViewBag.PageName = PageHelper.Pages.haberbulteni_index;
            ViewBag.PageProperties = PageHelper.PageProperties.SetPageProperties("Haber Bülteni", "", "Haber Bülteni İşlemleri", "");

            return View();
        }

        public ActionResult ExcelRaporuAl(string eposta, DateTime? baslangicTarihi, DateTime? bitisTarihi, int aktiflik)
        {
            var sonucListesi = HaberBulteniAbonelikService.FindBy(x =>
        ...
        #region Ajax Methods
        [HttpGet]
        public JsonResult Ara(string eposta, DateTime? baslangicTarihi, DateTime? bitisTarihi, int aktiflik, int sayfaSayisi, int sayfaSirasi)
        {
            var tempList = HaberBulteniAbonelikService.FindBy(x =>
            (string.IsNullOrEmpty(eposta) || x.Eposta.Contains(eposta)) &&
            (
                (baslangicTarihi == null && bitisTarihi == null) ||
                (((baslangicTarihi != null && bitisTarihi != null) && (x.Tarih >= baslangicTarihi && x.Tarih <= bitisTarihi))) ||
            ...
            var sonucListesi = tempList.OrderBy(x => x.HaberBulteniAbonelikId).Skip((sayfaSirasi - 1) * sayfaSayisi).Take(sayfaSayisi).ToList();
